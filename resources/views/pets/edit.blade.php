<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-2xl text-teal-950 leading-tight flex items-center gap-2.5">
            <a href="{{ route('pets.show', $pet) }}" class="inline-flex items-center justify-center h-10 w-10 bg-teal-50/80 hover:bg-teal-50 text-teal-700 rounded-full transition-all border border-teal-100/30 shadow-2xs" title="Regresar a la ficha">
                <svg class="h-5 w-5 stroke-[2.5]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                </svg>
            </a>
            <span>📝 {{ __('Editar Mascota') }}: <span class="text-teal-650">{{ $pet->name }}</span></span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white/80 rounded-[2rem] shadow-xl shadow-teal-900/5 border border-teal-100/60 overflow-hidden backdrop-blur-md">
                <div class="p-6 sm:p-8 border-b border-teal-50 bg-teal-50/30">
                    <h3 class="text-lg font-extrabold text-teal-950">Actualizar Ficha de la Mascota</h3>
                    <p class="text-xs text-teal-650/80 mt-1 font-semibold">Modifica los datos del paciente según sea necesario.</p>
                </div>
                ...
                <form action="{{ route('pets.update', $pet) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $pet->name) }}" class="w-full px-4 py-3 rounded-xl border @error('name') border-rose-455 focus:ring-rose-500/20 focus:border-rose-500 @else border-teal-100 focus:ring-teal-500/20 focus:border-teal-500 @enderror bg-white text-teal-950 placeholder-teal-655/40 transition-all focus:outline-none focus:ring-2 font-semibold" placeholder="Ej. Toby" required>
                            @error('name')
                                <p class="text-xs text-rose-550 font-extrabold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5 stroke-[2.5]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="species" id="species" class="w-full px-4 py-3 rounded-xl border @error('species') border-rose-455 focus:ring-rose-500/20 focus:border-rose-500 @else border-teal-100 focus:ring-teal-500/20 focus:border-teal-500 @enderror bg-white text-teal-955 transition-all focus:outline-none focus:ring-2 font-semibold" required>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="breed" id="breed" value="{{ old('breed', $pet->breed) }}" class="w-full px-4 py-3 rounded-xl border @error('breed') border-rose-455 focus:ring-rose-500/20 focus:border-rose-500 @else border-teal-100 focus:ring-teal-500/20 focus:border-teal-500 @enderror bg-white text-teal-950 placeholder-teal-655/40 transition-all focus:outline-none focus:ring-2 font-semibold" placeholder="Ej. Golden Retriever" required>
                            @error('breed')
                                <p class="text-xs text-rose-550 font-extrabold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5 stroke-[2.5]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="date" name="birthdate" id="birthdate" max="{{ date('Y-m-d') }}" value="{{ old('birthdate', \Carbon\Carbon::parse($pet->birthdate)->format('Y-m-d')) }}" class="w-full px-4 py-3 rounded-xl border @error('birthdate') border-rose-455 focus:ring-rose-500/20 focus:border-rose-500 @else border-teal-100 focus:ring-teal-500/20 focus:border-teal-500 @enderror bg-white text-teal-955 transition-all focus:outline-none focus:ring-2 font-semibold" required>
                            @error('birthdate')
                                <p class="text-xs text-rose-550 font-extrabold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5 stroke-[2.5]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="number" name="weight" id="weight" step="0.01" min="0.01" value="{{ old('weight', $pet->weight) }}" class="w-full px-4 py-3 rounded-xl border @error('weight') border-rose-455 focus:ring-rose-500/20 focus:border-rose-500 @else border-teal-100 focus:ring-teal-500/20 focus:border-teal-500 @enderror bg-white text-teal-950 placeholder-teal-655/40 transition-all focus:outline-none focus:ring-2 font-semibold" placeholder="Ej. 12.5" required>
                            @error('weight')
                                <p class="text-xs text-rose-550 font-extrabold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5 stroke-[2.5]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                    ...
                    <div class="space-y-3">
                        <label class="block text-xs sm:text-sm font-extrabold text-teal-950">Foto de la Mascota</label>
                        
                        <div class="flex items-center gap-5 flex-col sm:flex-row">
                            @if($pet->photo)
                                <div class="relative group self-center">
                                    <div class="bg-gradient-to-tr from-teal-200 to-emerald-300 p-0.5 rounded-[1.2rem] shadow-sm">
                                    ...
                                    <span class="absolute -top-2.5 -right-2.5 px-2 py-0.5 rounded-full text-[3xs] font-black bg-teal-600 text-white shadow-sm border border-teal-400">Actual</span>
                                </div>
                            @else
                                <div class="h-20 w-20 rounded-2xl bg-teal-50 text-teal-500 border border-dashed border-teal-200 flex items-center justify-center font-bold text-2xl shadow-3xs self-center">
                                    🐾
                                </div>
                            @endif

                            <div class="flex-1 w-full">
                                <div class="flex justify-center px-6 pt-4 pb-5 border-2 border-dashed border-teal-100 hover:border-teal-300 rounded-[1.5rem] bg-teal-50/10 hover:bg-teal-50/20 transition-all">
                                    <div class="space-y-1.5 text-center">
                                        <svg class="mx-auto h-9 w-9 text-teal-650/40" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                                        ...
                                        <p class="text-3xs text-teal-655/50 font-semibold">Dejar vacío para conservar la foto actual</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-teal-50">
                        <a href="{{ route('pets.show', $pet) }}" class="px-5 py-3 border border-teal-100 text-teal-700 font-extrabold rounded-xl hover:bg-teal-50/50 transition-all text-sm shadow-2xs">
                            Cancelar
                        </a>
