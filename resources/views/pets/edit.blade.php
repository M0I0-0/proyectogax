        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-800/50 overflow-hidden">
                <div class="p-8 border-b border-gray-100 dark:border-gray-800/50 bg-gray-50/50 dark:bg-gray-950/20">
                    <h3 class="text-lg font-bold text-gray-800 dark:text-white">Actualizar Ficha de la Mascota</h3>
                    <p class="text-xs text-gray-500 mt-1 font-medium">Modifica los datos del paciente según sea necesario.</p>
                </div>

                <form action="{{ route('pets.update', $pet) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $pet->name) }}" class="w-full px-4 py-3 rounded-xl border @error('name') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @else border-gray-200 dark:border-gray-800 focus:ring-emerald-500 focus:border-emerald-500 @enderror bg-gray-50/50 dark:bg-gray-950/50 text-gray-800 dark:text-gray-100 placeholder-gray-400 transition-all focus:outline-none focus:ring-2" placeholder="Ej. Toby" required>
                            @error('name')
                                <p class="text-xs text-rose-500 font-semibold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="breed" id="breed" value="{{ old('breed', $pet->breed) }}" class="w-full px-4 py-3 rounded-xl border @error('breed') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @else border-gray-200 dark:border-gray-800 focus:ring-emerald-500 focus:border-emerald-500 @enderror bg-gray-50/50 dark:bg-gray-950/50 text-gray-800 dark:text-gray-100 placeholder-gray-400 transition-all focus:outline-none focus:ring-2" placeholder="Ej. Golden Retriever" required>
                            @error('breed')
                                <p class="text-xs text-rose-500 font-semibold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="date" name="birthdate" id="birthdate" max="{{ date('Y-m-d') }}" value="{{ old('birthdate', \Carbon\Carbon::parse($pet->birthdate)->format('Y-m-d')) }}" class="w-full px-4 py-3 rounded-xl border @error('birthdate') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @else border-gray-200 dark:border-gray-800 focus:ring-emerald-500 focus:border-emerald-500 @enderror bg-gray-50/50 dark:bg-gray-950/50 text-gray-800 dark:text-gray-100 transition-all focus:outline-none focus:ring-2" required>
                            @error('birthdate')
                                <p class="text-xs text-rose-500 font-semibold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" name="weight" id="weight" step="0.01" min="0.01" value="{{ old('weight', $pet->weight) }}" class="w-full px-4 py-3 rounded-xl border @error('weight') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @else border-gray-200 dark:border-gray-800 focus:ring-emerald-500 focus:border-emerald-500 @enderror bg-gray-50/50 dark:bg-gray-950/50 text-gray-800 dark:text-gray-100 placeholder-gray-400 transition-all focus:outline-none focus:ring-2" placeholder="Ej. 12.5" required>
                            @error('weight')
                                <p class="text-xs text-rose-500 font-semibold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                    ...
                    <div class="space-y-3">
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300">Foto de la Mascota</label>
                        
                        <div class="flex items-center gap-5">
                            @if($pet->photo)
                                <div class="relative group">
                                    <img src="{{ asset('storage/' . $pet->photo) }}" class="h-20 w-20 rounded-2xl object-cover border border-gray-100 dark:border-gray-800/50 shadow-md transition-all group-hover:scale-105" alt="Foto actual">
                                    <span class="absolute -top-1.5 -right-1.5 px-2 py-0.5 rounded-full text-3xs font-extrabold bg-emerald-500 text-white shadow-sm">Actual</span>
                                </div>
                            @else
                                <div class="h-20 w-20 rounded-2xl bg-indigo-50 dark:bg-indigo-950/40 text-indigo-500 border border-dashed border-indigo-200 dark:border-indigo-850 flex items-center justify-center font-bold text-2xl">
                                    🐾
                                </div>
                            @endif

                            <div class="flex-1">
                                <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-dashed border-gray-200 dark:border-gray-800/80 rounded-2xl bg-gray-50/50 dark:bg-gray-950/30">
                                    <div class="space-y-1 text-center">
                                        ...
                                        <p class="text-3xs text-gray-400">Dejar vacío para conservar la foto actual</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-800/50">
                        <a href="{{ route('pets.show', $pet) }}" class="px-6 py-3 border border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-850 font-bold transition-all text-sm">
                            Cancelar
                        </a>
