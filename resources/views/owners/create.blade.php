        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white/80 rounded-[2rem] shadow-xl shadow-teal-900/5 border border-teal-100/60 overflow-hidden backdrop-blur-md">
                <div class="p-6 sm:p-8 border-b border-teal-50 bg-teal-50/30">
                    <h3 class="text-lg font-extrabold text-teal-955">Información de Contacto del Dueño</h3>
                    <p class="text-xs text-teal-650/80 mt-1 font-semibold">Completa todos los campos para registrar un nuevo dueño en VetCare.</p>
                </div>
                ...
                <form action="{{ route('owners.store') }}" method="POST" class="p-6 sm:p-8 space-y-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full px-4 py-3 rounded-xl border @error('name') border-rose-455 focus:ring-rose-500/20 focus:border-rose-500 @else border-teal-100 focus:ring-teal-500/20 focus:border-teal-500 @enderror bg-white text-teal-950 placeholder-teal-655/40 transition-all focus:outline-none focus:ring-2 font-semibold" placeholder="Ej. Juan Pérez" required>
                        @error('name')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <!-- Email -->
                        <div class="space-y-1.5">
                            <label for="email" class="block text-xs sm:text-sm font-extrabold text-teal-950">Correo Electrónico</label>
                            <input type="email" name="email" id="email" value="{{ old('email') }}" class="w-full px-4 py-3 rounded-xl border @error('email') border-rose-455 focus:ring-rose-500/20 focus:border-rose-500 @else border-teal-100 focus:ring-teal-500/20 focus:border-teal-500 @enderror bg-white text-teal-950 placeholder-teal-655/40 transition-all focus:outline-none focus:ring-2 font-semibold" placeholder="juan.perez@ejemplo.com" required>
                            @error('email')
                                <p class="text-xs text-rose-550 font-extrabold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5 stroke-[2.5]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="phone" id="phone" value="{{ old('phone') }}" class="w-full px-4 py-3 rounded-xl border @error('phone') border-rose-455 focus:ring-rose-500/20 focus:border-rose-500 @else border-teal-100 focus:ring-teal-500/20 focus:border-teal-500 @enderror bg-white text-teal-955 placeholder-teal-655/40 transition-all focus:outline-none focus:ring-2 font-semibold" placeholder="+56 9 1234 5678" required>
                            @error('phone')
                                <p class="text-xs text-rose-550 font-extrabold mt-1 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5 stroke-[2.5]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                    {{ $message }}
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-teal-50">
                        <a href="{{ route('owners.index') }}" class="px-5 py-3 border border-teal-100 text-teal-700 font-extrabold rounded-xl hover:bg-teal-50/50 transition-all text-sm shadow-2xs">
                            Cancelar
                        </a>
